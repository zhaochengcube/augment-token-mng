...
use crate::{AppState, storage::AntigravityDualStorage};
use std::sync::Arc;
use tauri::Manager;

async fn get_storage_manager(
    app_handle: &tauri::AppHandle,
) -> Result<Arc<AntigravityDualStorage>, String> {
    ...
    if let Some(manager) = manager_option {
        ...
        initialize_antigravity_storage_manager(app_handle, &state)
            .await
            .map_err(|e| format!("Failed to initialize Antigravity storage manager: {}", e))?;
        let guard = state.antigravity_storage_manager.lock().unwrap();
        guard
            .clone()
            .ok_or("Antigravity storage manager still not initialized".to_string())
    }
}
