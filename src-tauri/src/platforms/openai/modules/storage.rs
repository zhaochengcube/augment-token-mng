...
async fn get_storage_manager(
    app_handle: &tauri::AppHandle,
) -> Result<Arc<OpenAIDualStorage>, String> {
    ...
    if let Some(manager) = manager_option {
        ...
        crate::data::storage::initialize_openai_storage_manager(app_handle, &state)
            .await
            .map_err(|e| format!("Failed to initialize OpenAI storage manager: {}", e))?;
        let guard = state.openai_storage_manager.lock().unwrap();
        guard
            .clone()
            .ok_or("OpenAI storage manager still not initialized".to_string())
    }
}
