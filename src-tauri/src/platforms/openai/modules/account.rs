...
pub async fn fetch_quota_with_retry(
    ...
    if let Err(ref e) = result {
        if e.contains("401") || e.contains("unauthorized") {
            ...
            if let Some(refresh_token) = &account.token.refresh_token {
...
pub async fn refresh_token_if_needed(
    ...
    if !force && !oauth::token_needs_refresh(&account.token, refresh_window_secs) {
        return Ok(false);
    }

    let new_token = if force {
        let refresh_token_value = account
            .token
            .refresh_token
            .as_ref()
        ...
        TokenData::new(
            ...
            response
                .refresh_token
                .or_else(|| account.token.refresh_token.clone()),
            response.id_token.or_else(|| account.token.id_token.clone()),
            response.expires_in,
            now + response.expires_in,
            response.token_type,
        )
    } else {
        oauth::ensure_fresh_token_with_window(&account.token, refresh_window_secs).await?
    };

    if new_token.access_token == account.token.access_token
        && new_token.expires_at == account.token.expires_at
    {
        return Ok(false);
    }
