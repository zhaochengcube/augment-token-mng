use super::traits::{
    ...
    ServerAccountSyncResponse, StorageError, SyncableAccount, SyncableLocalStorage,
};
use super::{AccountDbMapper, GenericPostgreSQLStorage};
use chrono::Utc;
use std::collections::HashMap;
use std::sync::Arc;

/// SQLite 本地 + PostgreSQL 远端 双向存储
pub struct SQLiteDualStorage<T, M>
where
    T: SyncableAccount,
    M: AccountDbMapper<T>,
{
    local_storage: Arc<dyn SyncableLocalStorage<T>>,
    postgres_storage: Option<Arc<GenericPostgreSQLStorage<T, M>>>,
}

impl<T, M> SQLiteDualStorage<T, M>
where
    T: SyncableAccount,
    M: AccountDbMapper<T>,
{
...
impl<T, M> AccountStorage<T> for SQLiteDualStorage<T, M>
where
    T: SyncableAccount,
    M: AccountDbMapper<T>,
{
...
#[async_trait::async_trait]
impl<T, M> AccountSyncManager<T> for SQLiteDualStorage<T, M>
where
    T: SyncableAccount,
    M: AccountDbMapper<T>,
{
