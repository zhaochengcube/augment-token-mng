...
pub fn convert_legacy_token(
    ...
    let portal_url = legacy
        .get("portal_url")
        .and_then(|v| v.as_str())
        .map(|s| s.to_string());

    let email_note = legacy
        .get("email_note")
        .and_then(|v| v.as_str())
        .map(|s| s.to_string());
    ...
    let auth_session = legacy
        .get("auth_session")
        .and_then(|v| v.as_str())
        .map(|s| s.to_string());
    let suspensions = legacy.get("suspensions").cloned();
    let balance_color_mode = legacy
        .get("balance_color_mode")
        .and_then(|v| v.as_str())
        .map(|s| s.to_string());
    let skip_check = legacy.get("skip_check").and_then(|v| v.as_bool());

    let version = legacy.get("version").and_then(|v| v.as_i64()).unwrap_or(0);

    let session_updated_at = legacy
        .get("session_updated_at")
        .and_then(|v| v.as_str())
...
pub fn convert_to_legacy_format(token: &TokenData) -> serde_json::Value {
    ...
    map.insert(
        ...
        serde_json::Value::String(token.tenant_url.clone()),
    );
    map.insert(
    ...
    map.insert(
        ...
        serde_json::Value::String(token.created_at.to_rfc3339()),
    );
    map.insert(
        "updated_at".to_string(),
        serde_json::Value::String(token.updated_at.to_rfc3339()),
    );

    if let Some(portal_url) = &token.portal_url {
        map.insert(
            "portal_url".to_string(),
            serde_json::Value::String(portal_url.clone()),
        );
    }

    if let Some(email_note) = &token.email_note {
        map.insert(
            "email_note".to_string(),
            serde_json::Value::String(email_note.clone()),
        );
    }
    ...
    if let Some(tag_color) = &token.tag_color {
        map.insert(
            "tag_color".to_string(),
            serde_json::Value::String(tag_color.clone()),
        );
    }
    ...
    if let Some(auth_session) = &token.auth_session {
        map.insert(
            ...
            serde_json::Value::String(auth_session.clone()),
        );
    }
    ...
    if let Some(balance_color_mode) = &token.balance_color_mode {
        map.insert(
            "balance_color_mode".to_string(),
            serde_json::Value::String(balance_color_mode.clone()),
        );
    }

    if let Some(skip_check) = token.skip_check {
        map.insert(
            "skip_check".to_string(),
            serde_json::Value::Bool(skip_check),
        );
    }
    ...
    if let Some(session_updated_at) = token.session_updated_at {
        map.insert(
            ...
            serde_json::Value::String(session_updated_at.to_rfc3339()),
        );
    }
