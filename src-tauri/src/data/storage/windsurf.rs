...
/// Windsurf 本地存储类型别名
pub type WindsurfLocalStorage = GenericLocalStorage<Account>;

/// Windsurf PostgreSQL 存储类型别名
pub type WindsurfPostgreSQLStorage = GenericPostgreSQLStorage<Account, WindsurfAccountMapper>;

/// Windsurf 双层存储类型别名
pub type WindsurfDualStorage = GenericDualStorage<Account, WindsurfAccountMapper>;

#[tauri::command]
pub async fn windsurf_sync_accounts_to_database(
...
#[tauri::command]
pub async fn windsurf_sync_accounts_from_database(
    state: State<'_, AppState>,
) -> Result<AccountSyncStatus, String> {
    let storage_manager = {
        let guard = state.windsurf_storage_manager.lock().unwrap();
...
#[tauri::command]
pub async fn windsurf_bidirectional_sync_accounts(
    state: State<'_, AppState>,
) -> Result<AccountSyncStatus, String> {
    let storage_manager = {
        let guard = state.windsurf_storage_manager.lock().unwrap();
...
#[tauri::command]
pub async fn windsurf_sync_accounts(
    req_json: String,
    state: State<'_, AppState>,
...
pub async fn initialize_windsurf_storage_manager(
    ...
    let local_storage = Arc::new(WindsurfLocalStorage::new(app)?);
    ...
    let postgres_storage = {
        ...
        if let Some(db_manager) = db_manager_guard.as_ref() {
            Some(Arc::new(WindsurfPostgreSQLStorage::new(db_manager.clone())))
        } else {
            None
        }
    };

    let dual_storage = Arc::new(WindsurfDualStorage::new(
        local_storage,
        postgres_storage,
        false,
    ));

    *state.windsurf_storage_manager.lock().unwrap() = Some(dual_storage);
